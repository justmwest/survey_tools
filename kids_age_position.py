...
kid, age, pos = [pull_vals(i) for i in [kid_col, age_col, pos_col]]

# now return values of AGE for each position and gender.
kid_unis, pos_unis = [unique_responses(i) for i in [kid,pos]]
...
def count_em_up():
    ...
    for uni_p in pos_unis:
        for uni_k in kid_unis: # for each position cycle through the genders.
            ...
            for a, k, p in zip(age,kid,pos): # Then run through all values looking for double matches.
                if p == uni_p and k == uni_k:
                    if (p,k) in out_dict.keys():
                        out_dict[p,k].append(a)
                    else:
                        out_dict[p,k] = [a] # for the first one create a list. For all the rest, add an item.
    return out_dict
